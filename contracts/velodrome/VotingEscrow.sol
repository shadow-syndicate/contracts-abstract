...
import {IERC20} from "contracts/velodrome/interfaces/IERC20.sol";
import {IVeArtProxy} from "contracts/velodrome/interfaces/IVeArtProxy.sol";
import {IVotingEscrow} from "contracts/velodrome/interfaces/IVotingEscrow.sol";
...
contract VotingEscrow is IERC721, IERC721Metadata, IVotes {
    ...
    function _transferFrom(
        ...
        // Clear approval., or it may be wrong in some unexpected situations.
    ...
    /// @param _t Time to calculate the voting power at
    /// @return Voting power
    ...
    function getPastVotes(address account, uint timestamp) public view returns (uint) {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            uint tId = _tokenIds[i];
            // Use the provided timestamp to get the balance at that time
    ...
    function _delegate(address delegator, address delegatee) internal {
        /// @notice differs from `_delegate()` in `Comp.sol` to use `delegates` override method to get delegatee
