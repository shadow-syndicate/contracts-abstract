...
export default async function (hre: HardhatRuntimeEnvironment) {
    ...
    console.log(`Syncing Shop lots${isZkSyncNetwork(hre) ? ' (zkSync)' : ' (EVM)'}... 🔄`);
    ...
    // Get Shop contract address (env var overrides config)
    const shopAddress = process.env.SHOP_ADDRESS || config.contracts.shop;
    if (!shopAddress) {
        throw new Error("Shop contract address not configured. Either set SHOP_ADDRESS environment variable or update config.ts");
    }

    console.log(`Connected to Shop at: ${shopAddress}\n`);

    const shop = await ethers.getContractAt("Shop", shopAddress, signer);
    ...
    for (const lotConfig of SHOP_LOTS) {
        ...
        const expectedPriceInTrax = hre.ethers.parseEther(lotConfig.priceInTrax);
        const expectedPriceInTraxTurbo = hre.ethers.parseEther(lotConfig.priceInTraxTurbo);
        ...
        const expectedData = {
            priceInTrax: expectedPriceInTrax,
            priceInTraxTurbo: expectedPriceInTraxTurbo,
        ...
        if (!currentLot || currentLot.priceInTrax === 0n) {
            // Needs creation
            changes.push({
                lotId,
                action: 'create',
                expectedData,
                differences: [
                    `Price TRAX: ${lotConfig.priceInTrax}`,
                    `Price Turbo: ${lotConfig.priceInTraxTurbo}`,
            ...
            if (currentLot.priceInTrax !== expectedPriceInTrax) {
                differences.push(`Price TRAX: ${hre.ethers.formatEther(currentLot.priceInTrax)} → ${lotConfig.priceInTrax}`);
            }
            if (currentLot.priceInTraxTurbo !== expectedPriceInTraxTurbo) {
                differences.push(`Price Turbo: ${hre.ethers.formatEther(currentLot.priceInTraxTurbo)} → ${lotConfig.priceInTraxTurbo}`);
            }
        ...
        try {
            ...
            if (currentLot && currentLot.priceInTrax > 0n) {
                changes.push({
                    ...
                    currentLot,
                    differences: [
                        `Price TRAX: ${hre.ethers.formatEther(currentLot.priceInTrax)}`,
                        `Price Turbo: ${hre.ethers.formatEther(currentLot.priceInTraxTurbo)}`,
    ...
    for (const change of changes) {
        if (change.action === 'create') {
            ...
            const tx = await shop.createLot(
                change.lotId,
                change.expectedData!.priceInTrax,
                change.expectedData!.priceInTraxTurbo,
            ...
            // Delete by setting price to 0 and deadline to 0
            const tx = await shop.updateLot(
                change.lotId,
                0, // priceInTrax
                0, // priceInTraxTurbo
