...
export async function deployOnly(artifact: string, args: any[]) {
...
export async function deployAndVerify(artifact: string, args: any[], hre: HardhatRuntimeEnvironment) {
    const factory = await ethers.getContractFactory(artifact);
    const contract = await factory.deploy(...args);
    await contract.waitForDeployment();
    const address = await contract.getAddress();
    console.log("Deployed", artifact, "at", address);

    await verifyContract(address, args, hre);
